...
input int     MaxSameCcyExposure = 1;       // 同一通貨・同方向の建玉上限 (USDロング重ね玉防止)
...
void EvaluatePair(const int pair_index, int current_positions_unused)
{
   ...
   for(int mi = 0; mi < 9; mi++)
   {
      ...
      //--- 同一通貨・同方向のエクスポージャ上限 (【R2教訓】USDロング×3 の再発防止)
      //    yf_name は "USD/JPY" 形式 (スラッシュ区切り 7 文字)
      if(StringLen(yf_name) >= 7)
      {
         string ccy_base  = StringSubstr(yf_name, 0, 3);
         string ccy_quote = StringSubstr(yf_name, 4, 3);
         int expo = CountSameCcyExposure(ccy_base, ccy_quote, (direction == "long"),
                                         g_magic_min, g_magic_max);
         if(expo >= MaxSameCcyExposure)
         {
            if(Verbose)
               PrintFormat("[FXSignalEA] %s %s %s: 同一通貨・同方向の建玉 %d 件 >= 上限 %d - skip (重ね玉防止)",
                           yf_name, method, direction, expo, MaxSameCcyExposure);
            continue;
         }
      }
