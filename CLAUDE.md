...
| `cs` | ✅ 稼働(ドルストレート床) | `strategy_cs.py` (通貨強弱: 最強×最弱+トレンド一致+15Mパターン。USD/CAD・USD/CHF・NZD/USD・EUR/USD・USD/JPY のみ) |
| `mtf` | ⏸ EVゲートで停止中 | `strategy_mtf.py` (日足+4H一致+15M S/Aパターン。60d計測で-EV→+EV実証待ち) |
...
| MTF 日足4H一致 | `strategy_mtf.py` (日足+4H `analyze_timeframe` 一致 + `patterns.detect` S/A。週/1Hは参考)。`docs/MTF_METHOD.md` |
| CS 通貨強弱 | `strategy_cs.py` (`currency_strength` 8通貨ランキング + トレンド一致 + 15Mパターン)。`docs/CS_METHOD.md` |
| 推奨ロット | `config.ACCOUNT_BALANCE_JPY`/`RISK_PERCENT` (env/Repository Variables) → アラートに残高×1%の具体ロット表示 (`build_static._recommended_lot`) |
...
| [`docs/CS_METHOD.md`](docs/CS_METHOD.md) | CS(通貨強弱)手法 — R2勝ち筋のコード化・ドルストレート床 |
