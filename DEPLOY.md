...
GitHub Actions cron は 15 分間隔なので、それより速い値動きは見えません。
**Finnhub** または **OANDA Practice** を組み合わせると、ブラウザが 15 秒 / 3 秒ごとに
最新ティックを取得し、価格がキーレベル (PDH/PDL/Entry/SL/TP) を跨いだ瞬間に
ブラウザ通知 + ビープが鳴るようになります。

#### プロバイダ比較

| | **Finnhub** (推奨) | OANDA |
|---|---|---|
| 本人確認 | **不要** (メアド登録のみ) | 必要 (グローバル版で簡易、Japan 版はゴールド会員) |
| 更新頻度 | 15 秒/15 ペア | 3 秒/15 ペア |
| データ | OANDA tick (Finnhub が再配信) | OANDA tick (直接) |
| Vercel env 変数 | `FINNHUB_API_KEY` | `OANDA_API_TOKEN` + `OANDA_ACCOUNT_ID` |
| 日本居住者で使いやすさ | ★★★ | ★ (Japan 法人は条件あり) |

両方設定すると **Finnhub が優先**されます。

---

#### A) Finnhub セットアップ (5 分・本人確認不要)
1. <https://finnhub.io/register> でメアド + パスワードのみで登録
2. ダッシュボードの **API key** をコピー
3. Vercel Project → **Settings → Environment Variables** に追加:

| Key | Value | Environments |
|---|---|---|
| `FINNHUB_API_KEY` | コピーした API key | Production, Preview |

4. **Deployments → 最新 deploy → ⋯ → Redeploy** で反映

#### B) OANDA セットアップ (グローバル版が無理なら飛ばして OK)
1. <https://www.oanda.com/sign-up/> (`.com`、**`.jp` ではない**) で **Demo (Practice)** を選択
2. メール認証完了後、ログイン → **Manage API Access** → **Generate** で Personal Access Token 発行
3. 同ページ上部の **Account ID** (例: `101-001-12345678-001`) もコピー
4. Vercel に追加:
...
| `OANDA_ENV` | `practice` (本番なら `live`) | Production, Preview |

> 日本居住者が `oanda.com` に登録できない場合は OANDA Japan のゴールド会員になるしかなく、
> その手間を避けるなら **Finnhub** を使ってください。

---

**重要**: `NEXT_PUBLIC_` を **付けない** こと (サーバ側のみ参照)。
ブラウザのバンドルにトークンが含まれないため漏洩リスクなし。

#### 動作確認
- フィルタバーの上に **「LIVE · Finnhub tick (OANDA 経由) · 15 ペア · 15 秒間隔」** (または OANDA tick) と緑のインジケータが出る
- 各通貨カードの現在値が **「LIVE · OANDA」** 表示になり、更新ごとに緑/赤フラッシュ
- 通貨をクリック → サイドバーに「ライブ価格」セクション + 各レベルへの距離 (pips)
- 価格がレベルを跨ぐと **ブラウザ通知 + ビープ音** (初回のみ Notification 許可を求められる)

#### コスト
両方とも完全無料。

| 項目 | Finnhub Free | OANDA Practice |
|---|---|---|
| API 料金 | 0 円 | 0 円 |
| レート制限 | 60 req/min | 30 req/sec |
| Vercel Functions 起動回数/月 | 約 17 万 (24h 稼働 1 ユーザー) | 約 86 万 (同上) |
| Vercel Free Tier 上限 | 1M | 1M |

Finnhub の方が Vercel Function 起動が少ないため、複数人で使うなら Finnhub 推奨。

#### よくある質問
- **本番口座 (OANDA Live) でも使える?**: `OANDA_ENV=live` にすれば叩けますが、本実装は価格取得のみで取引は行いません
- **15M トリガーが速くなる?**: いいえ。**シグナル計算は引き続き GitHub Actions の 15 分 cron**。
  ライブ価格は「キーレベル接近・突破」の検知だけを担当します
- **iOS Safari でも通知?**: 残念ながら iOS Safari の Web Notification はホーム画面追加した PWA のみ対応。音 (ビープ) は鳴ります
- **更新頻度を変えたい**: `useLivePrices(symbols, { intervalMs: 5000 })` のように `frontend/src/app/page.tsx`
  でオプションを渡してください。ただし Finnhub の場合 15 秒未満にすると 60 req/min 制限に抵触します
- **Finnhub が止まったら OANDA に自動切替?**: いいえ、現状は静的フォールバック。プロバイダ優先順位は
  `FINNHUB_API_KEY` → `OANDA_API_TOKEN`。両方設定して Finnhub の鍵を一時的に消すと OANDA にフェイルオーバーします
